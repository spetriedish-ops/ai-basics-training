...
It plays on softly crumpled ruled notebook paper that boils with the drawing.
...
8. adds one teal return arrow and a motion-plus-outline highlight so the loop
...
The generated audit assets in `source/` show the cleanup, every extraction
region, the final static layout, and the notebook background. They make it easy
to confirm that the lettering came from the photograph rather than a font or
redraw.
...
All crop boxes, layout positions, teaching-beat times, notebook colors, and
motion settings are near the top of `render_agentic_loop.py`. A spacing,
timing, paper, or contrast request normally takes 2–5 minutes to edit and a few
seconds to render on the machine used for the initial delivery. Replacing
handwritten wording takes about 5–10 minutes because the new linework must
first be added to the source photo or supplied as another photographed label.
